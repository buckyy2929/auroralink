...
> **Sourcing / scope note — पहले पढ़ो (Read first)**
>
> 1. **Aurora variant.** यह design `aurora_8b10b` v11.1 (Xilinx PG046) use करता है — **Aurora 64B/66B नहीं**। Specifically: 1 lane × 4-byte lane width, 2 Gbps line rate, 125 MHz GT refclk, 50 MHz `user_clk_out`, Framing mode, Duplex, Flow Mode = None। Source: `top.xsa/design_1.hwh`; full table in `docs/milestone2/M2-A_auroralink_findings.md` §2. इस document के पुराने drafts में जहाँ "Aurora 64B/66B" लिखा था, वो ग़लत था — clock math (2 Gbps × 8/10 ÷ 32 bits = 50 MHz user clock) सिर्फ़ 8B/10B के लिए consistent है।
> 2. **Frame rate (125 µs / 8 kHz).** यह rate RTDS power-system model से आता है, Aurora से नहीं। इस document में जो भी "125 µs / 8 kHz" है वो **assumed** है — authoritative RTDS docs citation अभी इस repo में नहीं है (open item — `docs/milestone2/M2-A_auroralink_findings.md` Known Gaps देखो)। हर "125 µs" / "8 kHz" mention को measured fact नहीं, order-of-magnitude expectation समझो — जब तक board पर inter-RC-interrupt period measure नहीं हो जाता।
> 3. **Hardware availability.** इस document के लिखने के time पर Kria board उपलब्ध नहीं था। हर "verified" claim किसी file path से sourced है (`top.xsa`, `Axi_IO.h`, etc.). हर "would happen on hardware" claim §14 (Bring-Up and Testing on KR260) में fenced है as pending board access।
>
...
│        ▼                                              │                  │   │
│  ┌────────────────┐                                   │  8B/10B encoder  │   │
│  │  Aurora 8B/10B│                                   │  CRC/scrambler   │   │
│  │  (RX side)     │                                   └──────────────────┘   │
│  │                │                                                          │
│  │  8b10b को वापस │  M_AXI_RX                                                │
│  │  AXI Stream    │ ─────────►┌─────────────┐                                │
...
- हर byte को **8B/10B line coding** से encode करता है (source: `top.xsa/design_1.hwh` — `aurora_8b10b` v11.1, PG046, 1 lane, 2 Gbps line rate)। 2 Gbps × 8/10 ÷ 32-bit lane = **50 MHz** user-side AXI-Stream clock। 64B/66B नहीं है।
...
Layer 2 — AuroraLink framing       Aurora IP add करता है: SOF marker, K-codes, CRC, idle codes
            ↓
Layer 1 — 8B/10B line coding       Aurora IP हर 8-bit byte को 10-bit symbol में map करता है (DC-balanced, embedded clock recovery)
            ↓
...
1. **Wire format** — fiber पर actually कौन से bits flow करते हैं। ये Aurora IP की configuration (line rate, lane count, line coding 8B/10B vs 64B/66B) से determine होता है। **इस design के लिए** configuration है `aurora_8b10b` v11.1 (PG046), 1 lane × 4 byte lane width, 2 Gbps line rate, 125 MHz refclk — sourced from `top.xsa/design_1.hwh`। दोनों ends पर इन settings पर agree होना चाहिए।
...
जब आप Vivado project create करते हो और **KR260** को board choose करते हो, फिर **Aurora 8B/10B** IP add करते हो, Vivado का IP integrator:
...
Aurora self-clock नहीं हो सकता — इसे external **GT reference clock** चाहिए। **इस design के लिए** (`aurora_8b10b` v11.1, 2 Gbps line rate, sourced from `design_1.hwh`) refclk **125 MHz** है, KR260 SOM के pins **Y6 / Y5** पर routed है (differential pair `GT_125_REFCLK_clk_p` / `GT_125_REFCLK_clk_n`)। KR260 पर:
...
Aurora 8B/10B IP wizard में (Vivado):
...
**हाँ।** Aurora 8B/10B link के पार AXI Stream framing preserve करता है। जो भी TVALID/TLAST patterns आप `s_axi_tx` पर put करते हो, वही `m_axi_rx` पर वापस मिलते हैं। ये पूरा point है Aurora का "framing interface" use करने का — ये transparent है।
...
| **GT reference clock** | **125 MHz** (per `top.xsa/design_1.hwh` — `C_REFCLK_FREQUENCY=125`, KR260 pins **Y6/Y5**) | KR260 board oscillator | Aurora का GTH PLL |
| **Aurora user clock** (`user_clk_out`) | **50 MHz** (per `design_1.hwh` — 2 Gbps × 8/10 ÷ 32-bit lane से derived) | Aurora IP खुद (output) | Aurora का user-side AXI Stream port, RX FIFO `axis_aclk` भी drive करता है |
| **Aurora init clock** | **50 MHz** (per `design_1.hwh` — `C_INIT_CLK=50`) | PL clock generator | Aurora IP का internal init state machine |
...
- GTH ("engine room") अपनी GT reference clock पर 125 MHz पर चलता है।
...
> **ये एक clock-crossing FIFO है!** Pattern gen 100 MHz (pl_clk0) पर चलता है, Aurora user side 50 MHz (`user_clk_out`, derived from 2 Gbps line rate ÷ 32-bit lane × 8/10) पर। Data FIFO **asynchronous** (independent input/output clocks) configure होना चाहिए। FIFO IP configuration में set करो: "Clock Type" = "Independent Clocks"।

#### Aurora 8B/10B
- `init_clk` → `pl_clk0` (IP की internal init state machine के लिए)
- `gt_refclk1_p`, `gt_refclk1_n` → external **125 MHz** differential pair on KR260 (board pins **Y6/Y5** per `design_1.hwh`; KR260 board files install होने के बाद board flow automatically connect करता है जब Aurora IP को SFP+ board interface से associate किया जाता है)
...
| `rst_aurora_user` | Aurora का `user_clk_out` (इस design में 50 MHz) | Aurora के user clock पर चलने वाले blocks reset करता है |
...
TX FIFO → 0x3FC00000
    ↓
Aurora TX (bytes को 8b/10b symbols में encode, 2 Gbps पर serialise)
    ↓ loopback
...
| Aurora 8B/10B | "aurora 8b10b" | Configure करो `top.xsa/design_1.hwh` से match करने के लिए: AXI4-Stream user interface, 1 lane, 4-byte lane width, 2 Gbps line rate, 125 MHz refclk, 50 MHz init clock, Framing mode, Duplex, Flow Mode = None, GT channel X0Y4 (Quad X0Y1)। पूरी checklist `docs/milestone2/M2-A_auroralink_findings.md` §5 में है। |
...
1. `axis_data_fifo/M_AXIS` पर click → `aurora_8b10b/S_AXI_TX` पर drag।
...
- Constant.dout → `aurora_8b10b/loopback` connect करो।
...
2. `aurora_8b10b/M_AXI_RX` → `axi_fifo_mm_s/AXI_STR_RXD` (PG080 FIFO पर AXI Stream RX input) connect करो।
...
| KR260 board से External **125 MHz** (pins Y6/Y5) | Aurora `gt_refclk1_p/n` (board flow automatically connect करता है अगर Aurora को KR260 SFP+ board interface से map किया हो; frequency `top.xsa/design_1.hwh` `C_REFCLK_FREQUENCY=125` से आती है) |
...
- Multiple clock domains exist (**इस design के लिए**, per `design_1.hwh`): PS clock 100 MHz (pl_clk0), Aurora `user_clk_out` 50 MHz, GT reference clock 125 MHz, Aurora init clock 50 MHz।
...
**जवाब:** **2 Gbps** use करो। ये **free choice नहीं** है — existing RTDS CHIL reference bitstream (`top.xsa/design_1.hwh`) यही use करता है (`C_LINE_RATE=2`, `C_REFCLK_FREQUENCY=125`, `aurora_8b10b` PG046)। 5/10 Gbps पर जाने के लिए PG074 (64B/66B) पर switch करना पड़ेगा, refclk 156.25 MHz करना पड़ेगा, और RTDS partner के साथ silently interop नहीं होगा। Internal PMA loopback लिए rate self-consistent होना ही काफी है, लेकिन production 2 Gbps पर रहने से lab और field bitstream एक ही artefact रहती है।
...
**जवाब:** Step-by-step:
1. ILA से `gt_pll_lock` probe। 1 होना चाहिए। अगर 0 → reference clock present नहीं या गलत frequency। इस design के लिए `gt_refclk1_p/n` KR260 **125 MHz** differential pair **Y6/Y5** से wired होना चाहिए (`design_1.hwh` देखो)। KR260 board files install होने के बाद board flow ये connection automatically बना देता है।
...
| Aurora | Xilinx serial-link IP family। दो variants: **8B/10B** (PG046, up to ~6.6 Gbps) — इस design में use हो रहा है — और **64B/66B** (PG074, higher rates)। इस project के लिए specific configuration: `aurora_8b10b` v11.1, 2 Gbps, 125 MHz refclk, 1 lane (per `top.xsa/design_1.hwh`)। |
