...
> **Sourcing / scope notes — पहले पढ़ो (Read first)**
>
> 1. **Aurora variant.** Fiber link `aurora_8b10b` v11.1 (Xilinx PG046) use करती है, **Aurora 64B/66B नहीं**। Specifically: 1 lane, 4-byte lane width, 2 Gbps line rate, 125 MHz GT refclk, 50 MHz `user_clk_out`, Framing mode, Duplex, Flow Mode = None। Source: hardware handoff `design_1.hwh` (extracted from `top.xsa`); full parameter table — `docs/milestone2/M2-A_auroralink_findings.md` §2। इस document का जो भी पुराना draft "Aurora 64B/66B" कहता था वो ग़लत है — clock math (2 Gbps × 8/10 ÷ 32 bits = 50 MHz user clock, hwh से confirmed) सिर्फ़ 8B/10B के लिए consistent है।
> 2. **Frame rate (125 µs / 8 kHz).** यह rate RTDS power-system model से set होता है, Aurora link से नहीं। इस document में जो "125 µs / 8 kHz" बार-बार आता है वो supplied CHIL reference का **assumed** rate है; authoritative RTDS docs section अभी repo में नहीं मिला (open item — `docs/milestone2/M2-A_auroralink_findings.md` Known Gaps)। हर "125 µs" / "8 kHz" mention को order-of-magnitude expectation समझो, measured fact नहीं — जब तक (a) RTDS manual citation add न हो जाए, या (b) live board पर inter-RC-interrupt period measure न हो जाए।
> 3. **PS environment.** यहाँ का CHIL reference एक Vitis bare-metal application (`xllfifo_interrupt_example_1`) है जो A53 पर RAM से `.elf` के through directly चलता है। Protocol-translator deliverable A53 पर **Ubuntu** पर चलेगा; FIFO interrupt path UIO या kernel module के through re-implement होगा (M2-A §9.6 और `bridge/include/aurora_adapter.h` देखो)। C-level driver call shape (`XLlFifo_*` → `read()`/`mmap`) सिर्फ़ बदलेगी; data path same रहेगा।
>
> **एक आसान सी कहानी (पूरे document में इसे याद रखना):**
> - **RTDS** = एक Train Driver जो simulation step rate पर train भेजता है (assumed ~125 µs; pending RTDS docs citation per note 2)
> - **Aurora fiber link** = वो railway track जिस पर train चलती है (8B/10B-coded 2 Gbps serial, see note 1)
...
**इनके बीच connection:** एक fiber optic cable जो **Aurora 8B/10B** serial protocol use करती है।
...
Aurora 8B/10B IP Core (PL):
  ...
  2. 8B/10B line encoding reverse करता है (per `design_1.hwh` — यह design 64B/66B नहीं है; 10-bit symbols → 8-bit bytes mapping, de-scramble)
