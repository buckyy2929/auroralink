...
u32 ReceiveLength; // length of received packet in words
...
int XLlFifoInterruptExample(XLlFifo *InstancePtr, u16 DeviceId)
{
	...
	xil_printf("Please Start RTDS model now \n\r");

#if 0	/* Transmit the Data Stream */
    ...
    //wait for Rx
		...
		//Run controller here:::: note for RTDS loopback Tx_Len = Rx_Len

		Status = TxSend(InstancePtr, DestinationBuffer);
		if (Status != XST_SUCCESS){
			xil_printf("Transmission of Data failed\n\r");
			return XST_FAILURE;
		}
...
int TxSend(XLlFifo *InstancePtr, u32  *SourceAddr)
{
	...
	for (i=0; i < MAX_DATA_BUFFER_SIZE; i++)
		*(SourceAddr + i) = i;
#endif
	//NO_OF_PACKETS=64; MAX_PACKET_LEN=4;
	//for(i=0; i < NO_OF_PACKETS; i++){
	xil_printf("Receive Length: %d\r\n", ReceiveLength);
	for(i=0; i < ReceiveLength; i++){
		/* Writing into the FIFO Transmit Port Buffer */
		//for (j=0; j < MAX_PACKET_LEN; j++){ // commented out 1/3/23 -BDR
			xil_printf("iTxVacancy: %d\r\n", iTxVacancy(InstancePtr));
			if( XLlFifo_iTxVacancy(InstancePtr) ){
				XLlFifo_TxPutWord(InstancePtr,
					*(SourceAddr+i)); //+(i*4)+0:rxLen // modified 1/3/23 -BDR
					//*(SourceAddr+(i*MAX_PACKET_LEN)+j)); //+(i*4)+0:rxLen
			}
		//} // commented out 1/3/23 -BDR

	}

	/* Start Transmission by writing transmission length into the TLR */
//	XLlFifo_iTxSetLen(InstancePtr, (MAX_DATA_BUFFER_SIZE * WORD_SIZE));
	XLlFifo_iTxSetLen(InstancePtr, (ReceiveLength * WORD_SIZE));
